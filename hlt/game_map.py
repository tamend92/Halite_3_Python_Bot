...
class GameMap:
    ...
    def get_enemy_shipyards(self,game):
    #finds all enemy shipyard locations 
        enemy_shipyard_loc = []
        # gets list of all enemy player IDs
        enemy_players = game.get_enemy_players()

        for player in enemy_players:
            enemy_shipyard_loc.append(game.players[player].shipyard)

        return enemy_shipyard_loc
    ...
    def count_halite_avail(self):
        '''
        Counts halite left on the map during this turn
        Should only be run once per turn
        returns amount as an integer
        '''
        available_halite = 0
        for y in range(self.height):
            for x in range(self.width):
                available_halite += self[Position(x,y)].halite_amount

        return available_halite
